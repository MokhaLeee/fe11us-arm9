...
def main(args):
	try:
		ov = int(args[0], base = 0)
		offset = 0x1FFFFFF & int(args[1], base = 0)

	except IndexError:
		sys.exit(f"Usage: {sys.argv[0]} ov addr")

	addr = offset + 0x02000000
	name = f'ProcScr_Unk_{offset + 0x02000000:08X}'
	
	if ov == 0:
		offset = offset - 0x1A23E0
	elif ov == 2:
		offset = offset - 0x1E3560
	elif ov == 4:
		offset = offset - 0x204C20
	elif ov == 8:
		offset = offset - 0x204c20
	else:
		print(f"unknow base offset for ov{ov}")
		return

	print(f"struct ProcCmd {name}[] =")
	print("{")

	rom_name = f'rom/overlay/overlay_{ov:04d}.bin'
