...
BOOL FSi_SendCommand(FSFile * p_file, FSCommandType command)
{
    ...
    p_file->stat |= FS_FILE_STATUS_BUSY;
    {
        ...
        // ???
        {
            FSFileLink *const trg = &p_file->link;
            FSFile *list = (FSFile *)&p_arc->list;
...
FSArchive * const FS_FindArchive(const char * name, int name_len)
{
    ...
    OSIntrMode bak_psr = OS_DisableInterrupts();
    FSArchive * p_arc = fs_arch_st.arc_list;
...
BOOL FS_RegisterArchiveName(FSArchive * p_arc, const char * name, int name_len)
{
    ...
    {
        FSArchive * p_tail = fs_arch_st.arc_list;
        if (!p_tail)
        {
            fs_arch_st.arc_list = p_arc;
            fs_arch_st.current_dir_pos.arc = p_arc;
            fs_arch_st.current_dir_pos.pos = 0;
            fs_arch_st.current_dir_pos.index = 0;
            fs_arch_st.current_dir_pos.own_id = 0;
        }
        else
        {
            while (p_tail->next)
...
void FS_ReleaseArchiveName(FSArchive * p_arc)
{
    if (p_arc->name.pack)
    {
        ...
        p_arc->flag &= ~FS_ARCHIVE_FLAG_REGISTER;
        if (fs_arch_st.current_dir_pos.arc == p_arc)
        {
            fs_arch_st.current_dir_pos.arc = fs_arch_st.arc_list;
            fs_arch_st.current_dir_pos.pos = 0;
            fs_arch_st.current_dir_pos.index = 0;
            fs_arch_st.current_dir_pos.own_id = 0;
        }
